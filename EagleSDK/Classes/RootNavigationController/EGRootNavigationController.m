...
@interface NSArray<ObjectType> (RootNavigationController)
- (NSArray *)rt_map:(id(^)(ObjectType obj, NSUInteger index))block;
- (BOOL)rt_any:(BOOL(^)(ObjectType obj))block;
@end

@implementation NSArray (RootNavigationController)
...
static inline UIViewController *SafeUnwrapViewController(UIViewController *controller) {
    if ([controller isKindOfClass:[ContainerController class]]) {
        return ((ContainerController *)controller).contentViewController;
    }
    return controller;
}
...
__attribute((overloadable)) static inline UIViewController *RTSafeWrapViewController(UIViewController *controller,
                                                                                     ...
                                                                                     NSString *backTitle) {
    if (![controller isKindOfClass:[ContainerController class]]) {
        return [ContainerController containerControllerWithController:controller
...
- (void)dealloc
{
//    NSLog(@"ContainerController's dealloc");
    [self.contentViewController componentDealloc];
                         ...
                         backTitle:(NSString *)backTitle
{
    ...
    if (self) {
        ...
        self.containerNavigationController = [[ContainerNavigationController alloc] initWithNavigationBarClass:navigationBarClass
                                                                                                    toolbarClass:nil];
        if (yesOrNo) {
...
- (void)viewDidLoad
{
    ...
    if (self.containerNavigationController) {
        ...
        // fix issue #16 https://github.com/rickytan/RootNavigationController/issues/16
...
- (void)viewDidLayoutSubviews
{
    ...
    // remove the following to fix issue #16 https://github.com/rickytan/RootNavigationController/issues/16
    // !!!: 我们在loadView内替换了self.view，需要放出如下代码进行解决，至于打电话的问题再看
    self.containerNavigationController.view.frame = self.view.bounds;
}
...
@interface UIViewController (ContainerNavigationController)
@property (nonatomic, assign, readonly) BOOL rt_hasSetInteractivePop;
@end

@implementation UIViewController (ContainerNavigationController)
...
- (UITabBarController *)tabBarController
{
    ...
    EGRootNavigationController *navigationController = self.rt_navigationController;
    if (tabController) {
...
- (NSArray *)viewControllers
{
    if (self.navigationController) {
        if ([self.navigationController isKindOfClass:[EGRootNavigationController class]]) {
                  ...
                  animated:(BOOL)animated
{
    for (UIView *view in [UIApplication sharedApplication].keyWindow.subviews) {
        if ([view isKindOfClass:EGPopView.class]) {
            [(EGPopView *)view dismiss];
            break;
        }
    }
    if (self.navigationController) {
...
@interface EGRootNavigationController () <UINavigationControllerDelegate, UIGestureRecognizerDelegate>
...
@implementation EGRootNavigationController
...
{
    self = [super initWithRootViewController:[[ContainerController alloc] initWithContentController:rootViewController]];
    if (self) {
                  ...
                  animated:(BOOL)animated
{
    if (self.viewControllers.count > 0) {
        UIViewController *currentLast = SafeUnwrapViewController(self.viewControllers.lastObject);
                                                     ...
                                                     animated:(BOOL)animated
{
    ...
    [[super viewControllers] enumerateObjectsUsingBlock:^(__kindof UIViewController * obj, NSUInteger idx, BOOL * stop) {
        if (SafeUnwrapViewController(obj) == viewController) {
            controllerToPop = obj;
            *stop = YES;
        }
    }];
...
- (void)removeViewController:(UIViewController *)controller animated:(BOOL)flag
{
    ...
    [controllers enumerateObjectsUsingBlock:^(__kindof UIViewController * obj, NSUInteger idx, BOOL * stop) {
        if (SafeUnwrapViewController(obj) == controller) {
                    ...
                    animated:(BOOL)animated
{
    ...
    [EGRootNavigationController attemptRotationToDeviceOrientation];
