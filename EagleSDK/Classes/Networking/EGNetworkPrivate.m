...
void EGLog(NSString *format, ...) {
#ifdef DEBUG
    if (![EGNetworkConfig sharedConfig].debugLogEnabled) {
        return;
    }
    va_list argptr;
...
+ (void)addDoNotBackupAttribute:(NSString *)path {
    ...
    if (error) {
        EGLog(@"error to set do not backup attribute, error = %@", error);
    }
}
...
@implementation EGBaseRequest (RequestAccessory)

- (void)toggleAccessoriesWillStartCallBack {
    for (id<EGRequestAccessory> accessory in self.requestAccessories) {
...
@implementation EGBatchRequest (RequestAccessory)
...
@implementation EGChainRequest (RequestAccessory)
