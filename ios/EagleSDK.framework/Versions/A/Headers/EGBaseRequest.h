...
FOUNDATION_EXPORT NSString *const EGRequestValidationErrorDomain;

NS_ENUM(NSInteger) {
    EGRequestValidationErrorInvalidStatusCode = -8,
    EGRequestValidationErrorInvalidJSONFormat = -9,
};

///  HTTP Request method.
typedef NS_ENUM(NSInteger, EGRequestMethod) {
    EGRequestMethodGET = 0,
    EGRequestMethodPOST,
    EGRequestMethodHEAD,
    EGRequestMethodPUT,
    EGRequestMethodDELETE,
    EGRequestMethodPATCH,
};

///  Request serializer type.
typedef NS_ENUM(NSInteger, EGRequestSerializerType) {
    EGRequestSerializerTypeHTTP = 0,
    EGRequestSerializerTypeJSON,
};

///  Response serializer type, which determines response serialization process and
///  the type of `responseObject`.
typedef NS_ENUM(NSInteger, EGResponseSerializerType) {
    /// NSData type
    EGResponseSerializerTypeHTTP,
    /// JSON object type
    EGResponseSerializerTypeJSON,
    /// NSXMLParser type
    EGResponseSerializerTypeXMLParser,
};

///  Request priority
typedef NS_ENUM(NSInteger, EGRequestPriority) {
    EGRequestPriorityLow = -4L,
    EGRequestPriorityDefault = 0,
    EGRequestPriorityHigh = 4,
};
...
typedef void(^EGRequestCompletionBlock)(__kindof EGBaseRequest *request);

///  The EGRequestDelegate protocol defines several optional methods you can use
///  to receive network-related messages. All the delegate methods will be called
///  on the main queue.
@protocol EGRequestDelegate <NSObject>

@optional
///  Tell the delegate that the request has finished successfully.
///
///  @param request The corresponding request.
- (void)requestFinished:(__kindof EGBaseRequest *)request;

///  Tell the delegate that the request has failed.
///
///  @param request The corresponding request.
- (void)requestFailed:(__kindof EGBaseRequest *)request;

@end

///  The EGRequestAccessory protocol defines several optional methods that can be
...
@protocol EGRequestAccessory <NSObject>
...
///  EGBaseRequest is the abstract class of network request. It provides many options
///  for constructing request. It's the base class of `EGRequest`.
@interface EGBaseRequest : NSObject
...
///  `EGResponseSerializerType`. Note this value can be nil if request failed.
///
...
///  If you use `EGResponseSerializerTypeJSON`, this is a convenience (and sematic) getter
...
///  Default is nil.
@property (nonatomic, weak, nullable) id<EGRequestDelegate> delegate;
...
@property (nonatomic, copy, nullable) EGRequestCompletionBlock successCompletionBlock;

///  The failure callback. Note if this value is not nil and `requestFailed` delegate method is
///  also implemented, both will be executed but delegate method is first called. This block
///  will be called on the main queue.
@property (nonatomic, copy, nullable) EGRequestCompletionBlock failureCompletionBlock;
...
///  The priority of the request. Effective only on iOS 8+. Default is `EGRequestPriorityDefault`.
@property (nonatomic) EGRequestPriority requestPriority;

///  Set completion callbacks
- (void)setCompletionBlockWithSuccess:(nullable EGRequestCompletionBlock)success
...
///  HTTP request method.
- (EGRequestMethod)requestMethod;

///  Request serializer type.
- (EGRequestSerializerType)requestSerializerType;

///  Response serializer type. See also `responseObject`.
- (EGResponseSerializerType)responseSerializerType;
