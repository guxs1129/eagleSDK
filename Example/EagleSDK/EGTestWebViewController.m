...
#import "EGTestWebViewController.h"
#import <AFNetworking/AFNetworking.h>
@interface EGTestWebViewController ()<WebComponentDelegate>
@property(nonatomic,strong)NSArray *components;

@end

@implementation EGTestWebViewController
...
//    [part addHandlers:@[@"rightBarButtonItems",@"btn1Action"] withTarget:self];
    ...
    [self subscribe:@"hello" withHandler:@selector(bustest:)];
    
    [self subscribe:@"hello" withBlock:^(id msg) {
        NSLog(@"%s%@",__func__,msg);
    }];
    [self subscribe:@"hello" withBlock:^(id msg) {
        NSLog(@"this is a second blk");
    }];
    [self.view subscribe:@"hello" withBlock:^(id msg) {
        NSLog(@"%@",self.view);
    }];
    
    [self.view subscribe:@"hello1" withResultBlock:^id(id msg) {
        return @"this is callback";
    }];
    
    [self setEventBusDebug:YES];
}
...
-(void)callJS{
//    [self.view unsubscribeAll];
    [self.view unsubscribeForTopic:@"hello1"];
    [self post:@"hello" withOptions:@"hello world"];
    [self post:@"hello1" options:nil withResultBlock:^(id result) {
        NSLog(@"%@",result);
    }];
